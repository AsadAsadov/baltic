...
app.get(['/api/work-items/:identifier', '/api/works/:identifier'], wrap(async (req, res) => { const identifier = String(req.params.identifier || ''); if (identifier === 'reorder') return res.status(405).json({ok:false,error:'METHOD_NOT_ALLOWED'}); const isIdentifierUuid = isUuid(identifier); const key=cacheKey([`work:detail:${isIdentifierUuid ? 'id' : 'slug'}`, identifier]); const result=await getOrSetCache(key, CACHE_TTL.worksList, async()=>{ const item = isIdentifierUuid ? await prisma.workItem.findUnique({ where:{ id:identifier } }) : await findWorkBySlug(identifier); if(!item) return null; if(!item.active && req.session?.adminAuthenticated !== true) return { __privateInactive:true }; return workOut(item, { list:false }); }); if(result.data?.__privateInactive) return res.status(401).json({ok:false,error:'ADMIN_AUTH_REQUIRED'}); if(!result.data) return res.status(404).json({ok:false,error:'WORK_ITEM_NOT_FOUND'}); res.set('X-BC-Cache', result.cache); res.json(result.data); }));
...
app.delete('/api/home-section-images/:id', requireAdminWrite, wrap(async (req,res)=>{ const id=String(req.params.id); const item=await prisma.homeSectionImage.findUnique({where:{id}}); const deleted=await prisma.homeSectionImage.deleteMany({where:{id}}); if(item?.imageUrl) await safeDeleteUploadFamily(item.imageUrl); invalidateCache(['home-section-images']); res.json({ok:true, deleted:deleted.count});}));
