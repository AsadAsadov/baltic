...
const allowedAudioExt = new Set(['.mp3', '.wav', '.m4a', '.aac', '.ogg']);
...
const isAllowedAudioUpload = (file) => {
  const ext = getUploadExtension(file?.originalname || '');
  const type = String(file?.mimetype || '').toLowerCase().trim();
  return allowedAudioExt.has(ext) && (type.startsWith('audio/') || type === 'application/octet-stream');
};
...
function projectCacheKeys(project = {}) {
  return ['projects:list', project.id && `project:detail:id:${project.id}`, project.legacyId != null && `project:detail:id:${project.legacyId}`, project.slug && `project:detail:slug:${project.slug}`].filter(Boolean);
}
function workCacheKeys(work = {}) {
  return ['works:list', work.id && `work:detail:id:${work.id}`, work.slug && `work:detail:slug:${work.slug}`].filter(Boolean);
}
...
const adminTabDefaultVisibility = { dashboard:true, messages:true, projects:true, works:true, gallery:true, hero:true, homeImages:true, audio:true, ads:true };
...
const isAdminPrivateGet = (req) => req.method === 'GET' && (
  ...
  (req.path.startsWith('/api/audio-tracks') && req.query.admin === 'true') ||
...
async function createVideoPoster(sourcePath, publicUrl) {
  const posterUrl = videoPosterUrl(publicUrl);
  const posterPath = uploadUrlToPath(posterUrl);
  if (!posterPath) return null;
  await execFileAsync('ffmpeg', [
    '-y', '-ss', '0.25', '-i', sourcePath, '-frames:v', '1',
    '-vf', 'scale=960:-2:force_original_aspect_ratio=decrease', '-q:v', '3', posterPath
  ], { timeout: 45000, maxBuffer: 2 * 1024 * 1024 });
  clearUploadExistsCache(posterUrl);
  return existingUploadUrl(posterUrl) || null;
}
async function normalizeVideoForBrowser(sourcePath, publicUrl) {
  if (!/\.mov$/i.test(String(publicUrl || ''))) return { filePath: sourcePath, publicUrl };
  const mp4Path = sourcePath.replace(/\.mov$/i, '.mp4');
  const mp4Url = String(publicUrl).replace(/\.mov$/i, '.mp4');
  await execFileAsync('ffmpeg', [
    '-y', '-i', sourcePath,
  ...
  clearUploadExistsCache(mp4Url);
  return { filePath: mp4Path, publicUrl: mp4Url };
}
...
function bannerOut(b) {
  if (!b) return b;
  const mediaType = b.mediaType || b.type || 'image';
  const variants = mediaType === 'image' ? withImageVariants(b.mediaUrl || '') : null;
  const mediaUrl = variants?.medium || b.mediaUrl || '';
  return { id: b.id, legacyId: b.legacyId, active: b.active, type: mediaType, mediaType, src: mediaUrl, mediaUrl, originalMediaUrl: b.mediaUrl || '', imageVariants: variants, link: b.linkUrl || '', linkUrl: b.linkUrl || '', title: b.title || b.titleAz || '', titleAz: b.titleAz || b.title || '', width: b.width, height: b.height, duration: b.duration, displayOrder: b.displayOrder || 0, views: b.views, clicks: b.clicks, placement: normalizePlacement(b.placement), position: normalizePlacement(b.placement), createdAt: b.createdAt };
}
function bannerIn(b = {}) {
...
app.get('/api/projects/slug/:slug', wrap(async (req,res)=>{ const key=cacheKey(['project:detail:slug', req.params.slug]); const result=await getOrSetCache(key, CACHE_TTL.projectsList, async()=>{ const p=await findProjectBySlug(req.params.slug); if(!p) return null; return projectOut(p, { list:false }); }); if(!result.data) return res.status(404).json({ok:false,error:'PROJECT_NOT_FOUND'}); res.set('Cache-Control','public, max-age=60, stale-while-revalidate=300'); res.set('X-BC-Cache', result.cache); res.json(result.data); }));
app.get('/api/projects/:identifier', wrap(async (req,res)=>{ const identifier = String(req.params.identifier || ''); const isIdentifierUuid = isUuid(identifier); const key=cacheKey([`project:detail:${isIdentifierUuid ? 'id' : 'slug'}`, identifier]); const result=await getOrSetCache(key, CACHE_TTL.projectsList, async()=>{ const p=isIdentifierUuid ? await prisma.project.findFirst({ where:{ id: identifier, ...(req.session?.adminAuthenticated === true ? {} : { archived:false }) } }) : await findProjectBySlug(identifier); if(!p) return null; return projectOut(p, { list:false }); }); if(!result.data) return res.status(404).json({ok:false,error:'PROJECT_NOT_FOUND'}); res.set('X-BC-Cache', result.cache); res.json(result.data); }));
app.post('/api/projects', requireAdminWrite, wrap(async (req,res)=>{ const data = projectIn(req.body); if (data.errors) return res.status(400).json({ok:false,errors:data.errors}); data.slug = await uniqueSlug('project', data.titleAz, data.slug); { const project = projectOut(await prisma.project.create({ data })); invalidateCache(projectCacheKeys(project)); res.status(201).json({ ok: true, project, ...project }); } }));
app.put('/api/projects/:id', requireAdminWrite, wrap(async (req,res)=>{ const p=await prisma.project.findFirst({ where:idWhere(req.params.id) }); if(!p) return res.status(404).json({ok:false,error:'PROJECT_NOT_FOUND'}); { const data = projectIn(req.body, p); if (data.errors) return res.status(400).json({ok:false,errors:data.errors}); { const project = projectOut(await prisma.project.update({ where:{ id:p.id }, data })); invalidateCache(projectCacheKeys(project)); res.json({ ok: true, project, ...project }); } }; }));
app.delete('/api/projects/:id', requireAdminWrite, wrap(async (req,res)=>{ const p=await prisma.project.findFirst({ where:idWhere(req.params.id) }); if (p) await prisma.project.delete({ where:{ id:p.id }}); invalidateCache([...projectCacheKeys(p || {}),'works:list','gallery:list']); res.json({ ok:true }); }));
app.patch('/api/projects/:id/archive', requireAdminWrite, wrap(async (req,res)=>{ const p=await prisma.project.findFirst({ where:idWhere(req.params.id) }); if(!p) return res.status(404).json({ok:false,error:'PROJECT_NOT_FOUND'}); { const project = projectOut(await prisma.project.update({ where:{ id:p.id }, data:{ archived: req.body.archived ?? !p.archived }})); invalidateCache(projectCacheKeys(project)); res.json({ ok: true, project, ...project }); } }));
...
app.get(['/api/work-items/slug/:slug','/api/works/slug/:slug'], wrap(async (req,res)=>{ const key=cacheKey(['work:detail:slug', req.params.slug]); const result=await getOrSetCache(key, CACHE_TTL.worksList, async()=>{ const item=await findWorkBySlug(req.params.slug); if(!item) return null; return workOut(item, { list:false }); }); if(!result.data) return res.status(404).json({ok:false,error:'WORK_ITEM_NOT_FOUND'}); res.set('Cache-Control','public, max-age=60, stale-while-revalidate=300'); res.set('X-BC-Cache', result.cache); res.json(result.data); }));
app.get(['/api/work-items/:identifier', '/api/works/:identifier'], wrap(async (req, res) => { const identifier = String(req.params.identifier || ''); const isIdentifierUuid = isUuid(identifier); const key=cacheKey([`work:detail:${isIdentifierUuid ? 'id' : 'slug'}`, identifier]); const result=await getOrSetCache(key, CACHE_TTL.worksList, async()=>{ const item = isIdentifierUuid ? await prisma.workItem.findUnique({ where:{ id:identifier } }) : await findWorkBySlug(identifier); if(!item) return null; if(!item.active && req.session?.adminAuthenticated !== true) return { __privateInactive:true }; return workOut(item, { list:false }); }); if(result.data?.__privateInactive) return res.status(401).json({ok:false,error:'ADMIN_AUTH_REQUIRED'}); if(!result.data) return res.status(404).json({ok:false,error:'WORK_ITEM_NOT_FOUND'}); res.set('X-BC-Cache', result.cache); res.json(result.data); }));
app.post(['/api/work-items', '/api/works'], requireAdminWrite, wrap(async (req,res)=>{
  ...
  try {
    ...
    invalidateCache(workCacheKeys(created));
    res.status(201).json(workOut(created, { list:false }));
  } catch (err) {
...
app.put(['/api/work-items/:id', '/api/works/:id'], requireAdminWrite, wrap(async (req,res)=>{ const existing=await prisma.workItem.findUnique({where:{id:String(req.params.id)}}); if(!existing) return res.status(404).json({ok:false,error:'WORK_ITEM_NOT_FOUND'}); const v=validateWorkBody(req.body, existing); if(v.errors) return res.status(400).json({ok:false,errors:v.errors}); const updated=await prisma.workItem.update({where:{id:existing.id},data:v.data}); invalidateCache([...workCacheKeys(existing), ...workCacheKeys(updated)]); res.json(workOut(updated, { list:false })); }));
app.patch(['/api/work-items/:id/archive', '/api/works/:id/archive'], requireAdminWrite, wrap(async (req,res)=>{ const w=await prisma.workItem.findUnique({where:{id:String(req.params.id)}}); if(!w) return res.status(404).json({ok:false,error:'WORK_ITEM_NOT_FOUND'}); const archived = req.body.archived ?? !w.archived; const updated=await prisma.workItem.update({where:{id:w.id},data:{archived}}); invalidateCache([...workCacheKeys(w), ...workCacheKeys(updated)]); res.json(workOut(updated, { list:false })); }));
app.patch(['/api/work-items/:id/status', '/api/works/:id/status'], requireAdminWrite, wrap(async (req,res)=>{ const w=await prisma.workItem.findUnique({where:{id:String(req.params.id)}}); if(!w) return res.status(404).json({ok:false,error:'WORK_ITEM_NOT_FOUND'}); const updated=await prisma.workItem.update({where:{id:w.id},data:{active:req.body.active ?? !w.active}}); invalidateCache([...workCacheKeys(w), ...workCacheKeys(updated)]); res.json(workOut(updated, { list:false })); }));
app.patch(['/api/work-items/:id/featured', '/api/works/:id/featured'], requireAdminWrite, wrap(async (req,res)=>{ const w=await prisma.workItem.findUnique({where:{id:String(req.params.id)}}); if(!w) return res.status(404).json({ok:false,error:'WORK_ITEM_NOT_FOUND'}); const updated=await prisma.workItem.update({where:{id:w.id},data:{featured:req.body.featured ?? !w.featured}}); invalidateCache([...workCacheKeys(w), ...workCacheKeys(updated)]); res.json(workOut(updated, { list:false })); }));
app.delete(['/api/work-items/:id', '/api/works/:id'], requireAdminWrite, wrap(async (req,res)=>{ const w=await prisma.workItem.findUnique({where:{id:String(req.params.id)}}); if(w){ await prisma.workItem.delete({where:{id:w.id}}); await safeDeleteUnusedWorkFiles([w.coverImage, ...jsonArray(w.images)], w.id); } invalidateCache(workCacheKeys(w || {})); res.json({ok:true}); }));
...
app.post('/api/uploads', requireAdminWrite, upload.single('file'), wrap(async (req, res) => {
  ...
  if (!isAllowedUploadFile(req.file, bucket)) return res.status(400).json({ ok: false, error: 'UNSUPPORTED_FILE_TYPE', message: 'Dəstəklənən şəkil, video və audio formatlarından istifadə edin.' });
  ...
  if (isAllowedVideoUpload(req.file) && /\.mov$/i.test(filename)) {
    try {
      const normalized = await normalizeVideoForBrowser(originalFilePath, publicUrl);
      filePath = normalized.filePath;
      publicUrl = normalized.publicUrl;
    } catch (error) {
      console.warn('[uploads:video-normalize]', { message: error?.message, bucket });
    }
  }
  ...
  const posterUrl = isAllowedVideoUpload(req.file)
    ...
    : null;
  res.status(201).json({ ok: true, bucket, path: objectPath, publicUrl, variants, posterUrl });
}));
...
const ADMIN_ASSET_VERSION = '20260725-5';
