...
function galleryOut(g) { return { id: g.id, src: g.mediaUrl, mediaUrl: g.mediaUrl, images: jsonArray(g.images), title: g.titleAz, titleAz: g.titleAz, titleRu: g.titleRu, titleEn: g.titleEn, type: g.type, archived: g.archived, sortOrder: g.sortOrder, createdAt: g.createdAt, updatedAt: g.updatedAt }; }
...
app.get('/api/projects', wrap(async (req,res)=>{ if (req.query.includeArchived === 'true' && req.session?.adminAuthenticated !== true) return res.status(401).json({ ok:false, error:'Admin girişi tələb olunur' }); res.json((await prisma.project.findMany({ where: req.query.includeArchived === 'true' ? {} : { archived:false }, orderBy:{ id:'desc' }})).map(p => projectOut(p, { list: true }))); }));
...
app.get(['/api/work-items', '/api/works'], wrap(async (req, res) => {
  ...
  res.json((await prisma.workItem.findMany({ where, orderBy:[{sortOrder:'asc'},{featured:'desc'},{createdAt:'desc'}] })).map(w => workOut(w, { list: true })));
}));
