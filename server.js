...
app.get('/api/projects/:id', wrap(async (req,res)=>{ const key=cacheKey(['project:detail:id', req.params.id]); const result=await getOrSetCache(key, CACHE_TTL.projectsList, async()=>{ const p=await prisma.project.findFirst({ where:idWhere(req.params.id) }); if(!p) return null; return projectOut(p, { list:false }); }); if(!result.data) return res.status(404).json({ok:false,error:'PROJECT_NOT_FOUND'}); res.set('X-BC-Cache', result.cache); res.json(result.data); }));
